...
def incrementa_horas(turmas,horas):
	...
	if horas:
		for it in turmas.alunos_inscritos.all():
			it.horas_faltadas = it.horas_faltadas + int(horas.pop(0))
			it.save()

def atualiza_situacao(turmas, situacao):

	if situacao:
		for it in turmas.alunos_inscritos.all():
			it.situacao = situacao.pop(0)
			it.save()
...
def remove_aluno(request, query, turmas):
	for a in query:
		...
		for matricula in turmas.alunos_inscritos.all():
			if matricula.aluno.numero_matricula == aluno.numero_matricula:
				turmas.alunos_inscritos.remove(matricula)
...
def remove_aluno_espera(request, remover, turmas):
	for b in remover:
			aluno = Aluno.objects.get(pk=b)
			for matricula in turmas.alunos_espera.all():
				if matricula.aluno.numero_matricula == aluno.numero_matricula:
					turmas.alunos_espera.remove(matricula)
					messages.success(request,  aluno.nome +' foi removido da lista de espera da turma ' + turmas.curso.nome + '-' + turmas.codigo)
					break

def cria_matricula(turma, aluno):
	matricula = Matricula()
...
def mover_aluno(request, mover, turmas):
	for b in mover:
			...
			if(turmas.alunos_inscritos.count() < turmas.vagas_presencial):
				matricula = busca_matricula(turmas.alunos_espera.all(), aluno)
				matricula.situacao = "M"
				matricula.save()
				turmas.alunos_espera.remove(matricula)
				turmas.alunos_inscritos.add(matricula)
...
def details_turma(request, pk):
	...
	context['turmas'] = turmas
	matriculas = turmas.alunos_inscritos.all()
